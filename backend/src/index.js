...
const getUserFromToken = async (req, res, next) => {
  // The token verification should now happen in auth.js route
  next();
};
...
app.post('/api/booking', getUserFromToken, async (req, res) => {
  const { environment_id, start_date, end_date, purpose, user_id } = req.body;
  
  try {
    // Validate required fields
    if (!environment_id || !start_date || !end_date || !purpose || !user_id) {
      console.error('Missing required fields:', { environment_id, start_date, end_date, purpose, user_id });
      return res.status(400).json({ 
        error: 'Missing required fields',
        details: { environment_id, start_date, end_date, purpose, user_id }
      });
    }

    const result = await db.query(
      'INSERT INTO booking (environment_id, user_id, start_date, end_date, purpose) VALUES ($1, $2, $3, $4, $5) RETURNING *',
      [environment_id, user_id, start_date, end_date, purpose]
    );
